    ...
    <div class="user-list-container">
      ...
      <el-table
          ...
          <el-table-column
                  ...
                  align="center">
            <template slot-scope="scope">
              <el-button type="text" size="small" @click="showGroupNodes(scope.row.id)">查看</el-button>
              <el-button type="text" size="small" @click="changeGroupNodes(scope.row.id)">修改</el-button>
            </template>
          </el-table-column>
          <el-table-column
                  prop=""
      ...
      <el-dialog title="查看前端菜单" :visible.sync="groupMenuVisible" width="400px">
        <el-tree
          ...
          :render-content="renderContent2menu"
          >
        </el-tree>
      ...
      <el-dialog title="查看授权节点" :visible.sync="groupNodeVisible" width="400px">
        <el-tree
          :data="nodeData"
          :show-checkbox="nodeFlag"
          default-expand-all
          node-key="id"
          ref="tree"
          highlight-current
          :default-checked-keys="defaultNodeCheckedKeys"
          :render-content="renderContent2node"
        >
        </el-tree>
        <span slot="footer" class="dialog-footer">
          <el-button @click="groupNodeVisible = false">取 消</el-button>
          <el-button type="primary" @click="submitGroupNodes">确 定</el-button>
        </span>
      </el-dialog>
    </div>
</template>
...
<script>
import { getGroupNodeList, updateGroupNode } from '@/api/resource'
...
export default {
  data() {
    return {
      ...
      groupNodeVisible: false,
      menuData: [],
      nodeData: [],
      defaultCheckedKeys: [],
      defaultNodeCheckedKeys: [],
      menuModifyData: {
        gid: 0, // 当前修改的组id
        mid: [] //
      },
  ...
  methods: {
    ...
    renderContent2menu(h, { node, data, store }) {
      // 用于格式化tree
      return (
    ...
    renderContent2node(h, { node, data, store }) {
      // 用于格式化tree
      return (
        <span style='flex: 1; display: flex; align-items: center; justify-content: space-between; font-size: 14px; padding-right: 8px;'>
          <span>
    ...
    showGroupNodes(gid) {
      this.nodeData = []
      this.nodeFlag = false
      this.groupNodeVisible = true
      getGroupNodeList(gid).then(res => {
        this.nodeData = res
      })
    },
    changeGroupNodes(gid) {
      this.nodeData = []
      this.nodeFlag = true
      this.groupNodeVisible = true
      this.nodeModifyData.gid = gid
      this.defaultNodeCheckedKeys = []
      getGroupNodeList(gid, { modify: true }).then(res => {
        this.nodeData = res.data
        this.defaultNodeCheckedKeys = res.permissions
      })
    },
    submitGroupNodes() {
      if (!this.nodeFlag) {
        this.groupNodeVisible = false
        return
      }
      console.log(this.$refs.tree.getCheckedKeys())
      updateGroupNode(this.nodeModifyData.gid, { nid: this.$refs.tree.getCheckedKeys() }).then(res => {
        this.groupNodeVisible = false
        this.$message({
          message: '操作成功',
          type: 'success'
